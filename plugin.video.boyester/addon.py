...
# xbmc hooks
__settings__ = xbmcaddon.Addon( id="plugin.video.boyester" )
...
def indexVideos(url):
    ...
    match=re.compile('u=http://www.boyester.com(/videos/.*/?).*" target="_blank" rel="nofollow"><img src="(.*\.jpg)" alt="(.*)" width').findall(link)
...
def playVideo(url,name,thumb):
    ...
    response.close()
    print link
    match=re.compile('video_url: \'([A-Za-z0-9_/.:-?&=]*)\',').findall(link)
    print match
...
base_url='http://www.boyester.com/'
